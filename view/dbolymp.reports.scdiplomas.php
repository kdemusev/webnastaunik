<div class="mainframe">
  <div class="subheader">Рейтинг учреждений по количеству дипломов</div>
  ...
  <form class="transpform" name="formform"
        action="/dbolymp/reportscdiplomas" method="post">
  ...
  <div id="toprint">
  	<div class="printable">
  		...
  		<h1 align="center">Рейтинг учреждений по количеству дипломов по результатам проведения II этапа
    ...
    <table class="smptable overablerow">
          ...
          <th>Педагоги, подготовившие победителей</th>
  			</tr>
  		</thead>
  		<?php $lastcnt = -1; $i = 0; if(count($data)) { foreach($data as $rec) { ?>
  			<tr>
  				<td align="center">
          ...
          <td>
  					<?php foreach($datateachers[$rec['olymp_school_id']] as $rec2) { ?>
              <?=$rec2['otname']?> (<?=$rec2['moldiploma']?>)<br />
            <?php } ?>
  				</td>
  			</tr>
  		<?php } } else { ?>
