<div class="mainframe">
    ...
    <div class="subheader">Премирование из фонда экономии</div>

    <?php CTemplates::showMessage('updated', 'Изменения сохранены'); ?>

    <form class="transpform" name="formform" action="/bonussalary/economy" method="post">
      <label>Период:</label>
  		<select name="bspdate">
    ...
    <form class="transpform" action="/bonussalary/saveeconomy" method="post">
      ...
      <label>Премиальный фонд экономии:</label>
      <input type="text" name="bsppurseeconomy" value="<?=isset($_bspdata[0]) ? $_bspdata[0]['bsppurseeconomy'] : ''?>" id="bppursebonus" onkeyup="recalculatetotal();" />
      ...
      <div class="extable" style="height: 350px; overflow: scroll;" onscroll="setColsBack();" id="scrollContainer">
            ...
            <?php $i = 1; foreach($_bsedata[$recg['id']] as $rec) { ?>
              <tr>
                <?php foreach($_bsadata as $rec2) { ?>
                  <td style="width: 45px;" nowrap align="center"><input type="text" class="input" style="width: 10px;" name="bsvalue[<?=$rec['id']?>][<?=$rec2['id']?>]"
        ...
        <div style="position: absolute; right: 0; top: 0; width: 130px; z-index: 2;" id="rightPanel">
          <table>
            ...
            <tr>
              <td><input type="text" sc2group="<?=$recg['id']?>" class="input" id="sum_<?=$rec['id']?>" style="text-align: right;"
                    onchange="this.style.backgroundColor = 'yellow';" name="bspays[<?=$rec['id']?>" readonly onkeyup="upAndDown(this, event);" /></td>
            </tr>
            <?php } ?>

            <tr>
              <td><input type="text" class="input" id="groupsum<?=$recg['id']?>" style="text-align: right; font-weight: bold;" readonly /></td>
            </tr>
          </table>
        </div>
      </div>
