<div class="mainframe">
  ...
  <div class="subheader">Добавление участника конкурса</div>

  <form class="transpform">
    <input type="button" name="newtask" onclick="back();" value="Вернуться без сохранения" />
  </form>

  <form class="transpform" action="/dbconcurs/savepupil/<?=$data['ctid']?>" method="post"
        name="addForm">
    ...
    <p>Педагог: <b><?=$data['otname']?></b></p>
    <p>Название работы: <b><?=$data['cnname']?></b></p>

    <h2>Добавленные ранее участники</h2>
    <?php $i = 1; foreach($data_pupils as $rec) { ?>
      <p><?=$i++?>. <?=$rec['opname']?> (<?=$rec['ofname']?> класс)</p>
    <?php } ?>
